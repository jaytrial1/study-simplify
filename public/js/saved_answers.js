...
function enhanceCodeBlocks(messageElement, chartData) {
    ...
    imageCards.forEach(card => {
        ...
        if (!imageUrlFromData) {
            console.warn('[DEBUG] ai-image-card (saved-answers) found without data-image-url:', card);
            return;
        }
        ...
        if (!finalImageUrl.toLowerCase().startsWith('http') && !finalImageUrl.startsWith('/')) {
            ...
            console.log('[DEBUG] Relative path detected (saved-answers), converted to absolute:', finalImageUrl);
        }
        console.log('[DEBUG] ai-image-card (saved-answers) original imageUrl:', imageUrlFromData, 'finalImageUrl:', finalImageUrl, card);
